...
map_datpct_2 <- read.csv(file.choose(), header = TRUE)
...
q5_dat <- dat %>%
  select(REGION, EGEOLOC , SUD_Type, q3_age) %>%
  filter(q3_age == "Adult")



q5_dat_1 <- q5_dat %>%
 group_by(q3_age, REGION, EGEOLOC, SUD_Type)  %>%
      summarise(n = n()) %>%
      mutate(pct = n / sum(n)) %>%
        filter(SUD_Type == "Cocaine")

q5_dat_1 <- q5_dat_1 %>%
          select(q3_age, REGION, EGEOLOC, SUD_Type, pct) %>%
            set_names("q3_age", "region", "EGEOLOC", "SUD_Type", "pct")
...
map_datapct <- merge(us_state_map, q5_dat_1, by ='region', all = T) %>%
          filter(!is.na(region)) %>%
            filter(!is.na(long)) %>%
              filter(!is.na(lat)) %>%
                filter(!is.na(group)) %>%
                  filter(!is.na(order)) %>%
                   filter(!is.na(q3_age)) %>%
                    filter(!is.na(EGEOLOC)) %>%
                      filter(!is.na(SUD_Type)) %>%
                        filter(!is.na(pct))
  
         
write.csv(map_datapct, "map_datpct.csv")  


## plot
p <- ggplot(map_datpct_2, aes(x = long, y = lat, group = group))
p + geom_polygon(aes(fill = pct)) +
