...
### Question 4
  ...
  Most adolescents start using marijuana (3) then prescription drug use, this conflicts with the current findings, but most research focuses on the population as a whole. The Truven dataset covers only about 50% of the population as a result those who are particularly disadvantaged are not represented and the demographics and behavior of initial drug use are likely to be skewed.   
  Distinguishing between the young adult and adult population can be difficult at times. However, Schulte reported that young adults had the highest rates of medical emergencies due to drug use in comparison to other groups due to marijuana,a heroin, and amphetamines.
SAMHSA actually showed that heroin was used significantly more often than cocaine (2) conflicting with the proportions found, it also showed a stabilizing rate of use which has been corroborated in other reporting (1), (3), (4). The adult population proportional cocaine use seems similar, with significantly lower cocaine use rates than seen here and a more stabilized rate overall.
  Overall, the trend downwards for cocaine use is similar to other reports (3), but the proportion does not seem to be accurately representative although it should be noted that the Truven set only has approximately 50% of the population so generalizability is limited. The Truven data and the question 3 graph focused on amphetamines compared to cocaine so making inferences about proportional comparison must be taken judiciously, but the overall trend downwards is fairly accurate. Given that the graph focuses on comparing amphetamine to cocaine use the findings may balance out if other substance use disorders were incorporated into the comparison. 
...
States with the highest CUD Prevalence - Alabama, Colorado, Nebraska
States with the lowest CUD Prevalence - Massachusetts, Michigan, Tennessee
